...
#############Part1 Create datasets for two-sample MR analysis #############
...
##Part one--Prepare data for two-sample MR
...
for (exposure1 in exposures) {
  for (outcome1 in outcomes) {
    ...
    print(paste("step 2: extract SNPs from outcome dataset: ", nrow(outcome_dat), ". Data dimension：", dim(outcome_dat)[1], dim(outcome_dat)[2]))
    ######################
    ...
    print(paste("step 3: harmonise the exposure and outcome, keep：", nrow(e_o_dat_clean), "SNPs", ". e_o_dat_clean dimension：", dim(e_o_dat_clean)[1], dim(e_o_dat_clean)[2]))
    ##
    
    #step4: Variance explained by IV and IV power
    ...
    outdat_file <- paste0("./Forward_MR/data_beforeoutliers/", exposure1, "_", outcome1, "_V1.csv")
...
for (outcome in outcomes) {
  for (exposure in exposures) {
    ...
    input_file <- paste("./Forward_MR/data_beforeoutliers/", exposure, "_", outcome, "_V1.csv", sep = "")
    output_file <- paste("./Forward_MR/data_clean/", exposure, "_", outcome, "_V2.csv", sep = "")
...
for (exposure in exposures) {
  for (outcome in outcomes) {
    ...
    mydat <- read.delim(file = paste0("./Forward_MR/data_clean/", exposure, "_", outcome, "_V2.csv"), header = TRUE, sep = ",")
...
output_file_res <- paste0("./Forward_MR/results_main/Results_PAtoStroke.csv")
output_file_heter <- paste0("./Forward_MR/results_main/Heterogeneity.csv")
output_file_pleio <- paste0("./Forward_MR/results_main/MRegger_intercept.csv")
...
outdat_file_mrpresso <- paste0("./Forward_MR/sensitivity/MRPRESSO.csv")
outdat_file_outliers <- paste0("./Forward_MR/sensitivity/Outliers.csv")
outdat_file_distort <- paste0("./Forward_MR/sensitivity/MRPRESSO_distort.csv")
...
data <- read.table(paste0("./Forward_MR/results_main/MRPRESSO_distort.csv"), 
                   header = T, sep = ",", stringsAsFactors = FALSE)

data_long <- data %>%
  pivot_longer(cols = c(V1, V2), names_to = "variable", values_to = "value")

outdat_file_distort <- paste0("./Forward_MR/results_main/MRPRESSO_distort.csv")
...
output_file_res <- paste0("./Forward_MR/results_moveoutliers/Results_PAtoStroke_mo.csv")
output_file_heter <- paste0("./Forward_MR/results_moveoutliers/Heterogeneity_mo.csv")
output_file_pleio <- paste0("./Forward_MR/results_moveoutliers/MRegger_intercept_mo.csv")
